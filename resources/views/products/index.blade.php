            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @forelse ($products as $product)
                    <div class="card overflow-hidden">
                        ...
                        @else
                            <div class="w-full h-40 bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-400">No Image</div>
                        @endif
                        <div class="p-4 space-y-2">
                            <a href="{{ url('/products/'.$product->slug) }}" class="block font-semibold hover:text-primary">
                                {{ $product->name }}
                            </a>
                            @if (!empty($product->category))
                                <div class="badge-muted">{{ $product->category }}</div>
                            @endif
